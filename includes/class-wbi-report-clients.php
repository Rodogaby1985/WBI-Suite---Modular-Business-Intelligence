...
class WBI_Report_Clients {
    ...
    public function render() {
        $tab = isset($_GET['tab']) ? $_GET['tab'] : 'ranking';
        ...
        <div class="wrap">
            ...
            <div style="background:#fff; padding:15px; border:1px solid #c3c4c7; border-top:none; margin-bottom:15px;">
                <form method="get">
                    ...
                    al <input type="date" name="end" value="<?php echo $end; ?>">
            ...
            <div style="background:#fff; padding:20px; margin-top:10px; border:1px solid #c3c4c7;">
                <?php
                if($tab=='ranking'){
                    $top = $this->engine->get_clients_ranking('revenue', $start, $end);
                    echo "<h3>🏆 Top Clientes ({$start} al {$end})</h3>";
                    echo '<table class="widefat striped"><thead><tr><th>Nombre</th><th>Email</th><th>Total Gastado</th><th>Cant. Pedidos</th></tr></thead><tbody>';
                    if($top) foreach($top as $c) echo "<tr><td><strong>{$c->display_name}</strong></td><td>{$c->user_email}</td><td>".wc_price($c->total_val)."</td><td>{$c->count_val}</td></tr>";
                    ...
                    echo '<table class="widefat striped"><thead><tr><th>Nombre</th><th>Email</th><th>Última Compra</th></tr></thead><tbody>';
                    if($active) foreach($active as $a) echo "<tr><td>{$a->display_name}</td><td>{$a->user_email}</td><td>".date('d/m/Y', strtotime($a->last_buy))."</td></tr>";
                    ...
                    echo '<table class="widefat striped" style="max-width:500px;"><thead><tr><th>Ciudad</th><th>Nuevos Registros</th></tr></thead><tbody>';
                    if($zones) foreach($zones as $z) echo "<tr><td>".($z->city?:'Desconocido')."</td><td>{$z->count}</td></tr>";
